...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | Octopus86🐙")[0];
}
  ...
  (function ($) {
    $.fn.nekoScroll = function (option) {
      var defaultSetting = {
        ...
        borderRadius: 5 + 'px',
        right: 55.6 + 'px',
        nekoImg: "https://pic.imgdb.cn/item/65b69106871b83018ab4ce8d.png",
        hoverMsg: "瞅啥？",
...
function createtime1() {
  ...
  var ascll = [
    `欢迎光临Octopus86の居酒屋!`,
    `Welcome to Octopus86's Juju Restaurant 🍭🍭🍭`,
    `
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Octopus86🐙 %c 你正在访问Octopus86の居酒屋",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Octopus86🐙") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('Octopus86🐙的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://pic.imgdb.cn/item/65b68694871b83018a91c4f2.jpg);
